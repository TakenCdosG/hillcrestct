...
AnpsUpgrade::init();
/* CONSTANTS */
define('ANPS_TEMPLATE_LANG', 'constructo');
global $no_container;
$no_container = false;
    ...
    function vc_theme_rows($atts, $content) {
        ...
        if($coming_soon=="0"||is_super_admin()) { 
            ...
            elseif($atts['has_content']=="inside") { 
                ...
                /* Fullwidth Content */
                $no_container = true;
                return '<section '.$extra_id.' class="wpb_row row no-margin' . $extra_class . '">'.wpb_js_remove_wpautop($content).'</section>';
            }
        } else { 
...
function anps_scripts_and_styles() {
    ...
    //wp_enqueue_style("rtl-bootstrap", "//cdn.rawgit.com/morteza/bootstrap-rtl/master/dist/cdnjs/3.3.1/css/bootstrap-rtl.min.css");
    //transitions
    $transition_style = 'transition-'. get_option( 'logo_transition_style', '5' ) . '.css';
        //var_dump(get_option( 'logo_transition_style', $transition_style )); Exit;
    ...
    //wp_enqueue_script( "bootstrap-hover-dropdown", get_template_directory_uri()  . "/js/bootstrap-hover-dropdown.min.js", array('dropdown'), '', true );
...
function anps_theme_styles() {     
    ...
    if (get_option('font_source_1', "Google fonts")=='Google fonts') {
        wp_enqueue_style( "font_type_1",  'http://fonts.googleapis.com/css?family=' . get_option('font_type_1', 'Montserrat') . ':400italic,400,600,700,300&subset=latin,latin-ext');
    } else {
    ...
    if (get_option('font_source_navigation', "Google fonts")=='Google fonts' && get_option('font_type_1', 'Montserrat')!=get_option('font_type_2', 'PT+Sans') && get_option('font_type_1', 'Montserrat')!=get_option('font_type_navigation', "Montserrat")) {
        wp_enqueue_style( "font_type_navigation",  'http://fonts.googleapis.com/css?family=' . get_option('font_type_navigation', 'Montserrat') . ':400italic,400,600,700,300&subset=latin,latin-ext');
    }
