...
add_action('widgets_init',
     create_function('', 'return register_widget("wpfh_widget_obits_anniversary");')
);
class wpfh_widget_obits_anniversary extends WP_Widget{
	

	function __construct() {
		parent::__construct(
			'wpfh_obit_widget', // Base ID
			__('FuneralPress Anniversary', 'sp-wpfh'), // Name
	...
	public function widget( $args, $instance ) {
		...
		echo '<h2 class="widget-title">	'.$data['option1'].'</h2><div class="'. $data['option5'].'">';
		}
		if($atts['howmany'] != ""){
			$limit = $atts['howmany'];
			}else{
