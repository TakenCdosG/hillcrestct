...
class wpfh_obits_admin{
	...
	function guestbook($id= NULL){
		...
		global $wpdb;
			...
			$query = "SELECT * FROM  " . $wpdb->prefix . "wpfh_posts  WHERE oid = ".$wpdb->escape($id)." order by date desc";
...
<th>'.__("Action","sp-wpfh").'</th>
		...
		$approve = '<a href="admin.php?page=wpfh-guestbook&function=approve&id='.$r[$i]['id'].'&approve=1"  class="button"  style="margin-right:15px" >'.__("Approve","sp-wpfh").'</a>';		
					...
					if($r[$i]['uid'] == 0){
			$name = '<a href="mailto:'.$r[$i]['email'].'">'.$r[$i]['name'].'</a>';	
		}else{
	...
	if($_GET['id'] != "" or $_GET['function'] == 'add'){
	 ...
	 $this->add();
		...
		if($_POST['search-obits'] != ""){
			...
			$search = apply_filters("wpfh_admin_search_obit_query", $search);
		 ...
		 $r = $wpdb->get_results("".$query." ".$pagination->getLimitSql()."", ARRAY_A);
			...
			<form action="" method="post">
				...
				'.__("First Name","sp-wpfh").': <input type="text" name="first_name">   '.__("Last Name","sp-wpfh").': <input type="text" name="last_name"> '.__("Death Date","sp-wpfh").': <input type="text" name="date" class="datepicker">  <input type="submit" name="search-obits" value="'.__("Search!","sp-wpfh").'">
...
<td>'. date_i18n(get_option('date_format') ,strtotime( $r[$i]['death_date'])).'</td>';
if($r[$i]['approved'] == 1){
