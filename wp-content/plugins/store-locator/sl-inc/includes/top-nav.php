...
$show_my_views_button = ($sl_thanks!="true" && SL_THANKS_NUM_LOCS >= 1); //removed $sl_thanks!="false" 1-9-16 //2nd part of clause shows it to longer time users who haven't rated. For now, will show to newer users //|| ($sl_thanks=="false" && empty($sl_vars['thanks_num_locs']) && empty($sl_vars['thanks_time_length']) ));
...
if (empty($thnks_ct) || $thnks_ct<=20){
	...
	//unequal clauses come into play if dismissed until later time, but now either metric (# locs, time passed) has increased to next level
	$style_var .= ($tm_st && $nm_locs && $show_my_views_button && (SL_THANKS_TIME_LENGTH != $sl_vars['thanks_time_length'] || SL_THANKS_NUM_LOCS > $sl_vars['thanks_num_locs']) )? "jQuery('#thanks_button').click();\njQuery('.pp_close').css('visibility', 'hidden');\n" : "" ;
}
